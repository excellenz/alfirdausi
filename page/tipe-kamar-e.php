<!-- visitors -->
<div class="w3l-visitors-agile" >
  <div class="container">
    <h3 class="title-w3-agileits title-black-wthree">Kamar Tipe E
    </h3> 
  </div>
  <div class="w3layouts_work_grids">
    <section class="slider">
      <div class="flexslider">
        <ul class="slides">
          <li>
            <div class="w3layouts_work_grid_left">
              <img src="images/tipe-e.jpg" alt=" " class="img-responsive" />
              <div class="w3layouts_work_grid_left_pos">
                    <div class="embed-responsive embed-responsive-16by9">
                        <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/Gt3R3DQhPzU?rel=0"></iframe>
                    </div>
              </div>
            </div>
            <div class="w3layouts_work_grid_right">
              <h4>
                Deskripsi Kamar:
              </h4>
              <br>
              <ul>
                  <li> 1 Ranjang tingkat</li>
                  <li> 2 Springbed ukuran 200 x 200 cm dan atas 200  x 170  cm</li>
                  <li> Air Panas/Water Heater</li>
                  <li> Air Conditioner / AC </li>
                  <li> TV Flat 24'</li>
                  <li> Kloset Duduk</li>
                  <li> Shower</li>
                  <li> Gratis Wifi</li>
                  <li> Kamar Mandi dan Toilet di dalam</li>
                  <li> Lokasi di Luar Area HK di Lantai 2</li>
                  <li> Non Breakfast</li>
              </ul>
              <h5>Jumlah Kamar Kosong : 1</h5>
              <div class="agileits_w3layouts_more menu__item">
                <a href="index.php?page=tipe-kamar" >Kembali</a>
                <a href="manager/tamu/" >Pesan</a>
              </div>            
            </div>
            <div class="clearfix"> 
            </div>
          </li>
        </ul>
      </div>
    </section>
  </div>  
</div>
<!-- visitors -->
